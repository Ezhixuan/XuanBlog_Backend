...
public class MemoCardServiceImpl extends ServiceImpl<MemoCardMapper, MemoCard>
  ...
  @Resource private MemoDecksService decksService;
  ...
  public void operate(MemoCardOperateDTO operateDTO) {
    ThrowUtils.throwIf(
        Objects.isNull(operateDTO) || Objects.isNull(operateDTO.getId()), ErrorCode.PARAMS_ERROR);
  ...
  public IPage<MemoCardVO> getMemoPageVOList(MemoQueryDTO queryDTO) {
    ...
    IPage<MemoCard> iPage = queryDTO.toPage();
    page(iPage, lqw);
    if (CollectionUtils.isEmpty(iPage.getRecords())) {
      return new Page<>();
    }
    Set<Long> deckIds =
        iPage.getRecords().stream().map(MemoCard::getDeckId).collect(Collectors.toSet());
    Map<Long, String> nameMap =
        decksService.listByIds(deckIds).stream()
  ...
  public List<MemoCardVO> test(Long deckId) {
    ...
    List<Long> ids =
        listObjs(
            Wrappers.<MemoCard>lambdaQuery()
                .le(MemoCard::getNextReviewDate, LocalDateTime.now())
                .eq(Objects.nonNull(deckId), MemoCard::getDeckId, deckId)
                .select(MemoCard::getId),
            o -> (long) o);
  ...
  private void updatePlanWithSm17(Long quality, MemoCard memoCard) {
    ...
    if (memoCard.getLastReviewTime() != null) {
      actualIntervalDays =
          Sm17AlgorithmUtil.calculateDaysBetween(memoCard.getLastReviewTime(), now);
    }

    // 计算当前的理论检索能力
    double currentStability = memoCard.getStability();
    double theoreticalRetrievability =
        Sm17AlgorithmUtil.calculateRetrievability(currentStability, actualIntervalDays);
    ...
    double newDifficulty =
        Sm17AlgorithmUtil.updateDifficulty(
            memoCard.getDifficulty(), quality, theoreticalRetrievability, actualRetrievability);

    // 更新稳定性
    double newStability =
        Sm17AlgorithmUtil.updateStability(
            currentStability, newDifficulty, actualRetrievability, quality);

    // 计算下次复习的最优间隔
    long newInterval =
        Sm17AlgorithmUtil.calculateOptimalInterval(newStability, DEFAULT_FORGETTING_INDEX);
    ...
    log.info(
        "SM-17算法更新完成 - 卡片ID:{}, 质量:{}, 难度:{:.3f}, 稳定性:{:.2f}天, 间隔:{}分钟",
        id,
        quality,
        newDifficulty,
        newStability,
  ...
  private void updatePlanWithSm2(Long quality, MemoCard memoCard) {
    ...
    ThrowUtils.throwIf(
        Objects.isNull(id)
            || Objects.isNull(repetitions)
            || Objects.isNull(currentEaseFactor)
            || Objects.isNull(currentReviewInterval),
        ErrorCode.PARAMS_ERROR);

    // 1. 更新易度因子EF
    double calculatedNewEF =
    ...
    if (quality < 2L) {
      ...
      if (repetitions > 0
          && newInterval <= currentReviewInterval
          && Objects.isNull(reviewIntervalEnum)
          && quality >= 3) {
        newInterval = Math.round(currentReviewInterval * MIN_EASE_FACTOR);
        if (newInterval <= currentReviewInterval) newInterval = currentReviewInterval + 1;
      }
    ...
    log.info(
        "SM-2算法更新完成 - 卡片ID:{}, 质量:{}, 易度因子:{:.2f}, 间隔:{}分钟",
        id,
        quality,
        memoCard.getEaseFactor(),
        memoCard.getReviewInterval());
  }
  ...
  private void initializeSm17Fields(MemoCard memoCard) {
    ...
    // 初始化上次复习时间
    if (memoCard.getLastReviewTime() == null) {
      memoCard.setLastReviewTime(
          memoCard.getUpdateTime() != null
              ? memoCard.getUpdateTime()

...
class MemoDeckController {
  ...
  @GetMapping("/list")
  public PageResponse<MemoDeckVO> listAll() {
    List<MemoDeckVO> vos =
        decksService.getMemoVOList(
            decksService.list(
                Wrappers.<MemoDecks>lambdaQuery().orderByDesc(MemoDecks::getUpdateTime)));
    return R.list(vos);
  }
  ...
  public BaseResponse<String> del(OperationById idOpt) {
    ThrowUtils.throwIf(
        Objects.isNull(idOpt) || Objects.isNull(idOpt.getId()), ErrorCode.PARAMS_ERROR);
  ...
  public BaseResponse<String> add(@RequestBody MemoDeckDTO memoDeckDTO) {
    ThrowUtils.throwIf(
        Objects.isNull(memoDeckDTO) || Objects.isNull(memoDeckDTO.getName()),
